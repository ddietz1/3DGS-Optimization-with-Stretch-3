...
def _urdf_transform(xyz, rpy):
        """One URDF joint origin -> 4x4 transform. URDF rpy is intrinsic 'xyz'
        (roll, pitch, yaw in that order) -- confirmed against this project's
        own rpy_to_quaternion + as_euler('xyz') round-trip earlier this
        session."""
...
def _rz(angle):
    Tm = np.eye(4)
    Tm[:3, :3] = R.from_euler('z', angle).as_matrix()
    return Tm
...
_T_HEAD = _urdf_transform([0.0, 1.33, 0.0], [1.5707963267949, -1.5707963267949, 3.1416])
_T_PAN_ORIGIN = _urdf_transform(
    [0.13433950795041796, 0.07242074767659128, -0.00899316677053895],
    [-0.0018927008497759967, 0.00694949153505009, 1.5403829520062062])
_T_TILT_ORIGIN = _urdf_transform(
    [-0.00011858880721356138, 0.027352467730663958, -0.06302550314084383],
    [1.5765224323143903, 0.004705384140961444, 0.027644506223040044])
_T_CAMERA_JOINT = _urdf_transform(
    [0.02592053239520676, -0.009578372162050647, 0.017436941680225762],
    [0.004541542998557617, -0.002828213155519288, 0.016809505382889618])
_T_CAMERA_LINK_JOINT = _urdf_transform([0.010600000000000002, 0.0175, 0.0125], [0, 0, 0])
_T_CAMERA_COLOR_JOINT = _urdf_transform([0.0, 0.015, 0.0], [0, 0, 0])
_T_CAMERA_COLOR_OPTICAL_JOINT = _urdf_transform([0, 0, 0], [-1.5707963267948966, 0.0, -1.5707963267948966])
class MoveJoints(HelloNode):
    ...
    def __init__(self):
        ...
        if not self.has_parameter('run_num'):
            self.declare_parameter('run_num', 'holdout_run-07-27-5')
        if not self.has_parameter('initial_pose'):
            self.declare_parameter('initial_pose', [1.6, 0.0, 3.14])
        ...
        self.d435_poses = list(itertools.product(pans, tilts))  # -> every (pan, tilt) combo
        self.get_logger().info(f'd435 pans are {pans}')
        self.get_logger().info(f'd435 tilts are {tilts}')
        ...
        self.get_logger().info(f'yaw offsets are {self.yaw_offset}')
        ...
        initial_pose.pose.position.x = 1.6 # initial_pose_params[0]
        initial_pose.pose.position.y = 0.0 # initial_pose_params[1]
        ...
        self.test_IK_d435 = self.create_service(
            TestPoses,
            'test_d435_ik',
            self.test_d435_ik,
            callback_group=self.mode_cb_group
    ...
    def timer_cb(self):
        ...
        target_rot = R.from_quat([
            self.test_marker.orientation.x,
            self.test_marker.orientation.y,
            self.test_marker.orientation.z,
            self.test_marker.orientation.w
        ])
        x_to_z_correction = R.from_euler('y', -90, degrees=True)
        display_rot = target_rot * x_to_z_correction
        qx, qy, qz, qw = display_rot.as_quat()

        self.marker.pose = deepcopy(self.test_marker)
    ...
    # --- Testing Functions ---
    def test_d435_ik(self, request, response):
        self.move_to_pose({'joint_head_pan': 0.0, 'joint_head_tilt': 0.0}, blocking=True)
        ...
        self.get_logger().info(f'DEBUG: extracted target_yaw={degrees(target_yaw):.2f}')

        base_pose = self.compute_base_pose_for_camera_target(test_pose, target_yaw)
        if base_pose is None:
            self.get_logger().warn('Could not compute base pose for camera target')
            response.success = False
            return response

        # Move to navigation mode
        if self.stretch_mode != 'navigation':
            if not self.switch_mode(self.nav_mode_client, 'navigation'):
                self.get_logger().error('Could not switch to navigation mode; aborting D435 pose command')
                response.success = False
                return response
            
        self.navigator.goToPose(base_pose)
        while not self.navigator.isTaskComplete():
            time.sleep(0.2)
        if self.navigator.getResult() != TaskResult.SUCCEEDED:
            self.get_logger().warn('Could not reach solved base pose for d435 IK')
            response.success = False
            return response
        ...
        for attempt in range(max_reposition_attempts):
            try:
                achieved_tf = self.tf_buffer.lookup_transform(
                    'map', 'base_link', rclpy.time.Time(),
                    timeout=rclpy.duration.Duration(seconds=1.0))
            except Exception as e:
                self.get_logger().warn(f'Could not read back achieved base pose: {e}')
                break

            dx = base_pose.pose.position.x - achieved_tf.transform.translation.x
            dy = base_pose.pose.position.y - achieved_tf.transform.translation.y
            error = sqrt(dx ** 2 + dy ** 2)
            self.get_logger().info(
                f'Base position error after nav: {error:.3f}m '
                f'(attempt {attempt + 1}/{max_reposition_attempts})')
            if error <= max_base_position_error:
                break
            ...
            while not self.navigator.isTaskComplete():
                time.sleep(0.2)
            if self.navigator.getResult() != TaskResult.SUCCEEDED:
                self.get_logger().warn('Reposition attempt failed to complete')
                break
            time.sleep(0.5)
        else:
            self.get_logger().warn(
                f'Could not converge base position within {max_base_position_error}m '
                f'after {max_reposition_attempts} attempts -- capturing anyway, but this '
                f'candidate pose likely will not resolve its scored uncertainty.')

        # 2. Fine-tune pan/tilt for the exact final orientation match.
        pan, tilt = self.compute_d435_pan_tilt_for_orientation(test_pose)
        self.get_logger().info(f'DEBUG: computed pan={degrees(pan):.2f} tilt={degrees(tilt):.2f}')
        
        if pan is None or tilt is None:
            self.get_logger().warn('Could not solve pan/tilt for requested orientation')
            response.success = False
            return response
        ...
        # Vary pan and tilt slightly so that we can get multiple images at each pose
        for p in np.arange(-0.2, 0.2, 0.2):
            for t in np.arange(-0.3, 0.3, 0.3):
                self.move_to_pose({'joint_head_pan': pan+p, 'joint_head_tilt': tilt+t}, blocking=True)
                self.wait_until_settled(['joint_head_pan', 'joint_head_tilt'], vel_thresh=.005)
                time.sleep(0.5)

                pose_name = f'nbv_capture_{datetime.now().strftime("%Y%m%d_%H%M%S_%f")}'
                self.capture_frame(pose_name, camera='d435')
                tf = self.tf_buffer.lookup_transform('map', 'base_link', rclpy.time.Time())
                self.get_logger().info(f'ACTUAL base yaw after nav: {tf.transform.rotation}')   
    ...
    def synced_frame_cb(self, rgb_msg, depth_msg, info_msg):
        ...
        #self.get_logger().info(f'synced_frame_cb fired, stamp={rgb_msg.header.stamp}')
    ...
    def _amcl_cb(self, msg):
        """"Keeps track of the amcl pose for gating bad pose convergence."""

        self._latest_amcl_pose = msg
        # self.get_logger().info('captured new amcl pose')
    ...
    def compute_base_pose_for_camera_target(self, target_pose: PoseStamped, target_yaw: float):
        ...
        try:
            ...
            self.get_logger().warn(f'Could not get base->camera offset: {e}')
            return None

        mast_offset_local = np.array([
            tf.transform.translation.x, tf.transform.translation.y, tf.transform.translation.z,
        ])

        c, s = cos(target_yaw), sin(target_yaw)
        R_base = np.array([[c, -s], [s, c]])
        mast_offset_world_xy = R_base @ mast_offset_local[:2]

        base_x = target_pose.pose.position.x - mast_offset_world_xy[0]
        base_y = target_pose.pose.position.y - mast_offset_world_xy[1]

        base_pose = PoseStamped()
        base_pose.header.frame_id = 'map'
        base_pose.header.stamp = self.get_clock().now().to_msg()
        base_pose.pose.position.x = base_x
        base_pose.pose.position.y = base_y
        base_pose.pose.orientation.z = sin(target_yaw / 2.0)
        base_pose.pose.orientation.w = cos(target_yaw / 2.0)
        return base_pose
    ...
    def compute_d435_pan_tilt_for_orientation(self, target_pose):
        ...
        q = target_pose.pose.orientation
        target_rot = R.from_quat([q.x, q.y, q.z, q.w])
        desired_forward_map = target_rot.apply([0.0, 0.0, 1.0])  # camera +Z convention
        ...
        def cost(params):
            pan, tilt = params
            current_forward = self._base_to_camera_rotation(pan, tilt) @ np.array([0.0, 0.0, 1.0])
            return 1.0 - np.dot(current_forward, desired_forward_base)
    
        result = minimize(
            cost, x0=[0.0, 0.0], method='Nelder-Mead',
            bounds=[pan_limits, tilt_limits],
        ...
        if residual > 0.01:
            self.get_logger().warn(
                f'High residual ({residual:.4f}) solving D435 pan/tilt -- target '
                f'orientation may not be achievable from the current base pose'
            )
    ...
    def wait_for_localization_convergence(self, xy_variance_thresh=0.05, yaw_variance_thresh=0.06,
                                       timeout_sec=30.0, hold_sec=1.0, wiggle_interval_sec=5.0):
        start = self.get_clock().now()
        converged_since = None
        last_wiggle = self.get_clock().now()
        while True:
            if self._latest_amcl_pose is not None:
                cov = self._latest_amcl_pose.pose.covariance
                pos = self._latest_amcl_pose.pose
                self.get_logger().info(f'Waiting for convergence: x_var={cov[0]:.4f} y_var={cov[7]:.4f} yaw_var={cov[35]:.4f}')
                ok = cov[0] < xy_variance_thresh and cov[7] < xy_variance_thresh and cov[35] < yaw_variance_thresh
                if ok:
                    if converged_since is None:
                        converged_since = self.get_clock().now()
                    elif (self.get_clock().now() - converged_since).nanoseconds / 1e9 > hold_sec:
                        return True
                else:
                    converged_since = None
            ...
            if (now - start).nanoseconds / 1e9 > timeout_sec:
                self.get_logger().warn('Localization convergence timeout -- proceeding anyway')
                return False
            if (now - last_wiggle).nanoseconds / 1e9 > wiggle_interval_sec:
                self.get_logger().info('Still not converged -- injecting another small wiggle for fresh scan data')
                self.rotate_base(0.2)
                self.rotate_base(-0.2)
                last_wiggle = self.get_clock().now()
            time.sleep(0.2)
    ...
    def capture_frame(self, pose_name, timeout_sec=3.0, camera='d435'):
        ...
        # Log the acml covariance as well
        if self._latest_amcl_pose is not None:
            amcl_msg = self._latest_amcl_pose
            pose_record['amcl'] = {
                'position': [amcl_msg.pose.pose.position.x,
                            amcl_msg.pose.pose.position.y,
                            amcl_msg.pose.pose.position.z],
                'orientation': [amcl_msg.pose.pose.orientation.x, amcl_msg.pose.pose.orientation.y,
                                amcl_msg.pose.pose.orientation.z, amcl_msg.pose.pose.orientation.w],
                'covariance': list(amcl_msg.pose.covariance),  # full 6x6, flattened -- keep everything,
                # not just x/y variance, in case yaw variance or cross-terms matter later
            }
        else:
            pose_record['amcl'] = None
            self.get_logger().warn(f'No AMCL pose available when capturing {pose_name}')
    ...
    def command_pose_d435(self, target_pose: PoseStamped, standoff_dist=0.5):
        """Command the bot to go to a standoff position and take a photo with
        the d435, pointing it at target_pose. Mirrors command_pose_d405's
        structure -- navigate, then solve/apply camera orientation, then capture."""
        ...
        pan, tilt = self.compute_d435_pan_tilt(target_pose)
        self.get_logger().info(f'Solved d435 pan={pan:.3f}, tilt={tilt:.3f}')

        request = Camera.Request()
        response = Camera.Response()
        request.angles = [pan, tilt]
        request.joints = ['joint_head_pan', 'joint_head_tilt']
        self.move_camera(request=request, response=response, camera='d435')
        self.wait_until_settled(['joint_head_pan', 'joint_head_tilt'], vel_thresh=.005)
        time.sleep(0.5)

        self.capture_frame('nbv_test_capture', camera='d435')
        return True
    ...
    def follow_path(self, path, timeout_sec=60.0):
        ...
        # First we need to rotate and move the bot slightly so the acml will converge
        self.rotate_base(0.57)
        self.rotate_base(-0.57)
    ...
    def follow_waypoints(self):
        ...
        for wp_idx, (x, y, yaw) in enumerate(self.waypoints):
            ...
            # for y in self.yaw_offset:
            #     self.rotate_base()
            for idx, cp in enumerate(self.d435_poses):
                self.wait_for_localization_convergence(xy_variance_thresh=0.05, yaw_variance_thresh=0.05)
...
def main(args=None):
    ...
    open(os.path.join(node.capture_dir, 'DONE'), 'w').close()  # signals gpu_main_loop.py the initial batch is ready
