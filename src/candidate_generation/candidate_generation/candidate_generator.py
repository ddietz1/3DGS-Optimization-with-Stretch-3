    ...
    def check_pose_feasible(self, goal_pose_map: Pose) -> bool:
        ...
        try:
            ...
            goal_gap = math.hypot(
                ...
                goal_pose_map.position.y - last_pose.y,
            )
            if goal_gap > self.max_goal_endpoint_gap:
                self.get_logger().info(
                    f'Rejected candidate: planner path ends {goal_gap:.2f}m short of '
                    f'the requested goal (tolerance={self.max_goal_endpoint_gap}m) -- '
                    f'likely walled off, NavFn substituted a nearby reachable point'
                )
                return False
